// extra-screens.jsx — Calendar, Requests, Rewards, Settings

const { useState: useState_x, useRef: useRef_x } = React;

/* ============ Calendar ============ */
function Calendar({ openCall, openProfile }) {
  const [week, setWeek] = useState_x(0);
  ...
  return (
    <div className="page">
      <div className="page-head">
        <div className="left">
          <h1>Календарь</h1>
          <p>{HR_DATA.CALENDAR_EVENTS.length} событий на этой неделе · 16 стажёров на 1:1 за месяц</p>
        </div>
        <div className="right">
          <button className="btn" onClick={()=>window.toast?.("Календарь синхронизирован с Outlook · 16 событий", { kind:"good" })}>
            <Ihr.Swap size={14}/> Синхронизировать
          </button>
          <button className="btn primary" onClick={()=>window.toast?.("Открыто окно создания встречи", { sub:"Choose attendees & time" })}>
      ...
      <div className="cal-head">
        <div className="nav">
          <button className="icon-btn" onClick={()=>{ setWeek(w=>w-1); window.toast?.("Неделя 7 · 16–22 мая"); }}><Ihr.ArrowL size={14}/></button>
          <b>23 — 27 мая 2026</b>
          <button className="icon-btn" onClick={()=>{ setWeek(w=>w+1); window.toast?.("Неделя 9 · 30 мая – 5 июня"); }}><Ihr.Arrow size={14}/></button>
          <button className="btn ghost" style={{height:28, fontSize:12, marginLeft:8}} onClick={()=>{ setWeek(0); window.toast?.("К текущей неделе", { kind:"good" }); }}>Сегодня</button>
        </div>
        <div style={{display:'flex', gap:6}}>
          <button className="btn" style={{height:28, fontSize:12}} onClick={()=>window.toast?.("Просмотр: неделя")}>Неделя</button>
          <button className="btn ghost" style={{height:28, fontSize:12}} onClick={()=>window.toast?.("Просмотр: месяц")}>Месяц</button>
          <button className="btn ghost" style={{height:28, fontSize:12}} onClick={()=>window.toast?.("Просмотр: день")}>День</button>
        </div>
      </div>
      ...
      <div className="cal-grid">
        ...
        {hours.map(h => (
          <React.Fragment key={h}>
            ...
            {days.map((d, di) => {
              const evs = HR_DATA.CALENDAR_EVENTS.filter(e => e.day === di && Math.floor(e.start) === h);
              return (
                <div key={d+h} className="cal-cell">
                  {evs.map((e, ei) => {
                    ...
                    return (
                      <div key={ei} className={"cal-event " + e.kind}
                           style={{ top: topPct + "%", height: (dur * 56 - 4) + "px" }}
                           onClick={()=>window.toast?.(e.title, { sub: e.sub + " · " + formatTime(e.start) + "–" + formatTime(e.end) })}>
                        <b>{e.title}</b>
...
/* ============ Requests ============ */
function Requests() {
  const [tab, setTab] = useState_x("pending");
  const [items, setItems] = useState_x(HR_DATA.REQUESTS);

  function approve(id) {
    setItems(its => its.map(r => r.id === id ? { ...r, status: "approved" } : r));
    const r = items.find(x=>x.id===id);
    const e = HR_DATA.EMPLOYEES.find(x=>x.id===r.who);
    window.toast?.("Одобрено: " + r.kind.toLowerCase(), { sub: e.name, kind: "good" });
  }
  function deny(id) {
    setItems(its => its.filter(r => r.id !== id));
    window.toast?.("Заявка отклонена", { kind: "bad" });
  }
  function comment(id) {
    const r = items.find(x=>x.id===id);
    const e = HR_DATA.EMPLOYEES.find(x=>x.id===r.who);
    window.toast?.("Открыт диалог с " + e.name.split(' ')[0], { sub: "Можно запросить уточнение" });
  }
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <div className="right">
          <button className="btn" onClick={()=>window.toast?.("Фильтры применены")}><Ihr.Filter size={14}/> Фильтр</button>
          <button className="btn" onClick={()=>window.toast?.("CSV экспорт · 6 заявок", { kind: "good" })}><Ihr.Download size={14}/> Экспорт</button>
        </div>
      </div>
      ...
      <div className="req-list">
        ...
        {filtered.map(r => {
          const e = HR_DATA.EMPLOYEES.find(x=>x.id===r.who);
          return (
            <div className="req-row" key={r.id} onClick={()=>window.toast?.(r.kind, { sub: r.note })}>
              ...
              <div className="actions" onClick={(ev)=>ev.stopPropagation()}>
                ...
                </> : <button onClick={()=>window.toast?.("Решение зафиксировано в журнале", { kind: "good" })}>В журнал</button>}
              </div>
            </div>
          );
        })}
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <div className="right">
          <button className="btn" onClick={()=>window.toast?.("История наград за 90 дней", { sub: "Excel · 142 строки" })}><Ihr.Download size={14}/> История</button>
          <button className="btn primary" onClick={()=>window.toast?.("Открыт конструктор бейджа", { sub: "Настройте условия и XP-награду" })}><Ihr.Plus size={14}/> Создать бейдж</button>
        </div>
      </div>

      <div className="rwd-grid">
        {HR_DATA.REWARDS.map(r => (
          <div className="rwd-card" key={r.id}>
            ...
            <div className="foot">
              <button onClick={()=>window.toast?.("Открыт список получателей", { sub: r.name })}>
                <Ihr.Team size={12} style={{marginRight:4, verticalAlign:-1}}/> Получатели
              </button>
              <button onClick={()=>window.toast?.("Открыт конструктор · " + r.name)}>
                Настроить
              </button>
            </div>
          </div>
...
/* ============ Settings ============ */
function Settings() {
  const [tab, setTab] = useState_x("profile");
  const [toggles, setToggles] = useState_x({
  ...
  const tog = (k) => {
    const nv = !toggles[k];
    setToggles(t => ({ ...t, [k]: nv }));
    window.toast?.(nv ? "Включено: " + LABEL[k] : "Выключено: " + LABEL[k], { kind: nv ? "good" : "" });
  };
  const LABEL = {
    notif_dm:"Уведомления в DM", notif_group:"Уведомления в группах",
    notif_dl:"Уведомления о дедлайнах", notif_email:"E-mail дайджесты",
    rec_auto:"Авто-запись звонков", rec_consent:"Подтверждение записи",
  ...
  return (
    <div className="page">
      <div className="page-head">
        ...
        <div className="right">
          <button className="btn" onClick={()=>window.toast?.("Изменения отменены")}>Отменить</button>
          <button className="btn primary" onClick={()=>window.toast?.("Настройки сохранены", { kind: "good" })}>Сохранить</button>
        </div>
      </div>
          ...
          {tab === "profile" ? (
            ...
            <>
              ...
              {[
                ...
                <div className="settings-field" key={i}>
                  <div className="lbl"><b>{m.name}{m.you?<span className="tag cobalt" style={{marginLeft:8, fontSize:10}}>вы</span>:null}</b><small>{m.role}</small></div>
                  <div style={{display:'flex', gap:6}}>
                    <button className="btn" onClick={()=>window.toast?.("Открыт профиль · " + m.name)}>Профиль</button>
                    {!m.you ? <button className="btn" onClick={()=>window.toast?.("Открыт список разрешений", { sub: m.name })}>Доступ</button> : null}
                  </div>
                </div>
              ))}
            </>
            ...
            <>
              ...
              <div className="settings-field">
                ...
                <button className="btn" onClick={()=>window.toast?.("Открыт диалог смены пароля")}>Сменить пароль</button>
              </div>
              <div className="settings-field">
                <div className="lbl"><b>Активные сессии</b><small>4 устройства</small></div>
                <button className="btn" onClick={()=>window.toast?.("Выход на всех других устройствах", { kind: "good" })}>Завершить остальные</button>
              </div>
            </>
          ) : (
            <>
              ...
              <div className="settings-field">
                ...
                <button className="btn" style={{color:'var(--bad)', borderColor:'var(--bad)'}}
                        onClick={()=>window.toast?.("Запрос отправлен · требуется подтверждение по почте", { kind: "warn" })}>
                  Запросить очистку
                </button>
              </div>
            </>
