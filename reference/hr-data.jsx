// hr-data.jsx — mock data for HR portal

const AV_PALETTE = ['av-1','av-2','av-3','av-4','av-5','av-6','av-7','av-8'];
function avClassFromId(id) { return AV_PALETTE[id % AV_PALETTE.length]; }
...
].map(e => ({ ...e, initials: initials(e.name), avClass: avClassFromId(e.id) }));

const COHORTS = ["Розница", "Корпоратив", "Премиум"];
...
// Heatmap: cohorts × modules
const MODULES = ["Onboarding", "Розница: продукты", "Антифрод", "Корп. кредитование", "Эмпатия в звонке", "Эскалация"];
const HEATMAP = {
...
// Group conversations (with employees)
const GROUPS = [
  {
...
// Detailed thread for direct chat (Никита Соловьёв = id 1)
const THREAD_D1 = [
...
window.HR_DATA = {
  ...
  PERF_KPIS, MODULES, HEATMAP,
  GROUPS, DIRECT, THREAD_D1, SHARED_FILES,
  REQUESTS, REWARDS, CALENDAR_EVENTS,
