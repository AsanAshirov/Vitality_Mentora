// Clicky.jsx — cursor companion that rests in the side panel.
...
function useEasedFollow(target, smooth = 0.22) {
...
function Clicky({ enabled = true, mode = "cursor", target = null, message = null, onDismiss,
                  summoned, setSummoned,
                  autoSummoned = false,
                  color = "#2046FF" }) {
  const cursorPos  = useRef({ x: typeof window !== "undefined" ? window.innerWidth / 2 : 600, y: typeof window !== "undefined" ? window.innerHeight / 2 : 400 });
  const homePos    = useRef({ x: -100, y: -100 });
  const followTarget = useRef({ x: -100, y: -100 });
  ...
  useEffect(() => {
    if (mode !== "anchored" || !target || !effectiveSummoned) { setAnchorRect(null); return; }
    const update = () => {
    ...
    update();
    document.querySelectorAll(".main, .sim-main").forEach(n => n.addEventListener("scroll", update, { passive: true }));
    window.addEventListener("resize", update);
    const interval = setInterval(update, 350);
    return () => {
      document.querySelectorAll(".main, .sim-main").forEach(n => n.removeEventListener("scroll", update));
  ...
  if (effectiveSummoned && anchorRect && message) {
    ...
    if (placeRight)      bubble = { x: anchorRect.right + 18, y: anchorRect.y + anchorRect.h / 2, side: "right" };
    else if (placeLeft)  bubble = { x: anchorRect.x - 18,     y: anchorRect.y + anchorRect.h / 2, side: "left" };
    else                 bubble = { x: anchorRect.x + anchorRect.w / 2, y: anchorRect.bottom + 18, side: "bottom" };
  }
  ...
  return (
    <>
      ...
      {bubble && message && (
        <div className={`clicky-bubble side-${bubble.side}`}
             style={{ left: bubble.x, top: bubble.y }}
             role="dialog">
          <div className="who">
            <span className="live" style={{ background: color }} />
            {listening ? <>Клик · слушаю <span className="wave"><i/><i/><i/><i/><i/><i/></span></>
                       : <>Клик{message.who && ` · ${message.who}`}</>}
          </div>
          {message.lead && <p style={{ fontWeight: 500, marginBottom: 4 }}>{message.lead}</p>}
          <p dangerouslySetInnerHTML={{ __html: message.text }} />
          {onDismiss && <div className="actions"><button className="pill muted" onClick={onDismiss}>Понятно</button></div>}
        </div>
      )}

      <div className={`clicky-cursor ${effectiveSummoned ? "summoned" : "resting"}`}
           style={{ left: pos.x, top: pos.y, "--clicky-color": color }}>
        <div className={`clicky-mini ${listening ? "listening" : ""} ${effectiveSummoned && mode === "anchored" ? "thinking" : ""}`}>
